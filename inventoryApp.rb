...
post '/batch' do
    ...
    normalized_items.each_with_index do |item_name, idx|
      ...
      u = units[idx] || 0
      if exist_item
        exist_item.update(quantity: exist_item.quantity.to_i + qty, unit: exist_item.unit)
      else
...
post '/batch_edit' do
    require_login!
    itemnames = params[:batch_item] || []
    new_quantities = params[:batch_quantity] || []
    units = params[:batch_unit] || []

    itemnames.zip(new_quantities, units).each do |name, qty, u|
      item = Item.find_by(itemname: name)
      item.update(quantity: qty, unit: u) if item
    end
